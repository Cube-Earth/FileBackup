...
import earth.cube.tools.file_backup.commons.PositionedInputStream;
...
public class XattrFile implements Closeable {
	...
	public static boolean isValid(File file) throws IOException {
		if(file.getName().startsWith("._")) {
			if(CloseableAction.execute(new XattrFile(file), f -> f.isValid())) {
				return true;
			}
		}
		return false;
	...
	public static boolean hasLoweredTag(File file, String... saTag) throws IOException {
		if(file.getName().startsWith("._")) {
			if(CloseableAction.execute(new XattrFile(file), f -> f.isValid() && f.hasLoweredTag(saTag))) {
				return true;
			}
		}
		return false;
	...
	public static File getParentFile(File file) throws IOException {
		...
		return sName.startsWith("._") ? new File(file.getParentFile(), sName.substring(2)) : null;
	}
